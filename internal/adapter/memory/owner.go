...
func (r *InMemoryOwnerRepository) GetAll(ctx context.Context, limit, offset int) ([]*owner.Owner, error) {
	r.memory.Lock()
	defer r.memory.Unlock()

	if len(r.memory.owners) < offset {
		return nil, nil
	}

	return r.memory.owners[offset : offset+limit], nil
}
