...
func (j *Job) CancelApplication(applicationID int) error {
	if j.Status() != "open" {
		return ErrJobNotAvailable
	}
...
func (j *Job) AcceptApplication(applicationID int) error {
	if j.Status() != "open" {
		return ErrJobNotAvailable
	}
...
func (j *Job) RejectApplication(applicationID int) error {
	if j.Status() != "open" {
		return ErrJobNotAvailable
	}
...
func (j *Job) SetApplicationStatusToPending(applicationID int) error {
	if j.Status() != "open" {
		return ErrJobNotAvailable
	}
...
func (j *Job) GetApplicationByID(applicationID int) (entity.Application, error) {
	for _, a := range j.applications {
		if a.ID == applicationID {
			return *a, nil
		}
